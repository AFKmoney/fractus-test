...
- Note on execution: arms ran as separate detached processes (A and B in one run, C in a follow-up run after the first process was killed mid-C by an external timeout). All three use the same fixed corpus shuffle (seed 42) and the same engine init (seed 42), so the arms remain directly comparable.
