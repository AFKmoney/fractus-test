...
class Fractus1B(nn.Module):
    """Fractus1B: TRUE 1 billion trainable parameters.

    Config (default — config "K", measured at 0.956B params):
        vocab=50257, d_model=1280, n_layers=16, n_heads=20, d_head=64,
        n_levels=2, n_experts=128, top_k=2, expert_d_ff=2048, siren_rank=64.

    This is NOT a low-rank-compressed model pretending to be 1B. It is a full
    1B-parameter model using LazyStructuredSiren (rank 64) as its native
    weight storage. Every parameter is real, trainable, and counts toward
    the 1B total.

    RAM: ~4 GB (1B params × 4 bytes float32). Trainable on a single A100
    or a 48GB GPU (RTX 6000 Ada) with gradient accumulation.
    """
